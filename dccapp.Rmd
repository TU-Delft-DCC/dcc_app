  ...
  if (nrow(dt_selected()) != 0){  
    ...
    x %>% 
      hchart('column', hcaes(x = date_approx, y =  N  ), innerSize = 50,
             dataLabels = list(enabled = TRUE, position = 'center' ,  inside = T, style =
                                 list(textOutline = F) ) ) %>%
      hc_add_theme(dcc_hc_theme) %>%
      hc_yAxis(title = list(label = "Number of requests" , text = ""#"Number of applications"
                        )) %>%
...
<!--       hc_tooltip(crosshairs = TRUE, sort = TRUE, shared = TRUE, table = TRUE, -->
