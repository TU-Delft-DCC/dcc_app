...
packages <- c("flexdashboard", "remotes", "googlesheets4", "plotly", "bslib", "data.table", 
              "shiny", "shinydashboard", "shinyjs", "here", "tidyverse", "highcharter"
              )
...
#interesting plugin : selectsize: https://stackoverflow.com/questions/60831030/shiny-reactive-selectinput-in-r
 ...
 selectInput("dcc_support_round", label = "Select DCC support round:",
             choices = c('All', sort(unique(dt$dcc_support_round )))
            )

 selectInput("faculty", label = "Select Faculty:",
             choices = c('All', sort(unique(dt$faculty )))
            )
 
 selectInput('support_type', label='Select support type', 
             choices = c('All',sort(unique(dt$support_type)))
            ) 
 
 selectInput('status', label='Select support status', 
...
dt_selected<-reactive({
  dt_selected <- dt[(dcc_support_round %in% if(input$dcc_support_round == 'All') unique(dt[,dcc_support_round])  else input$dcc_support_round )
     & (faculty %in% if(input$faculty == 'All') unique(dt[,faculty])  else input$faculty )
     & (support_type %in% if(input$support_type == 'All') unique(dt[,support_type])  else input$support_type )
     & (status %in% if(input$status == 'All') unique(dt[,status])  else input$status )
     ]
...
renderPlotly({
  ...
  scale_y_continuous("Hours assgined", sec.axis = sec_axis(trans = ~./100, name = "Rainfall (mm)")) 
