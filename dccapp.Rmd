...
```{r filtered-data, context = "server"}
...
```{r}
flexdashboard::valueBoxOutput("vbox_nrows")
```

```{r vbox-nrows, context = "server"}

output$vbox_nrows <- flexdashboard::renderValueBox({
...
```{r}
flexdashboard::valueBoxOutput("vbox_completed")
```

```{r vbox-completed, context = "server"}

output$vbox_completed <- flexdashboard::renderValueBox({
...
```{r}
flexdashboard::valueBoxOutput("vbox_running")
```


```{r vbox-running, context = "server"}

output$vbox_running <- flexdashboard::renderValueBox({
...
```{r}
flexdashboard::valueBoxOutput("vbox_hrs")
```

```{r vbox-hrs, context = "server"}

output$vbox_hrs <- flexdashboard::renderValueBox({
...
```{r}
flexdashboard::valueBoxOutput("vbox_team")
```

```{r vbox-team, context = "server"}

output$vbox_team <- flexdashboard::renderValueBox({
...
```{r}
flexdashboard::valueBoxOutput("vbox_applicants")
```

```{r vbox-applicants, context = "server"}

output$vbox_applicants <- flexdashboard::renderValueBox({
...
```{r}
highchartOutput("round")
```

```{r lollipop-round, context = "server"}
# Useful highcharter reference: https://www.tmbish.me/lab/highcharter-cookbook/

output$round <- renderHighchart({
...
```{r}
highchartOutput("faculty")
```


```{r hcbar-faculty, context = "server"}

output$faculty <- renderHighchart({
